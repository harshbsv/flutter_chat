...
class ChatController extends GetxController {
  ...
  Stream chatData = FirebaseFirestore.instance
  ...
  DocumentReference lastmsgSetObject = FirebaseFirestore.instance
  ...
  sendMessage({to, message, personSent}) async {
    ...
    await messageObject
        ...
        .catchError((error) =>
            Get.rawSnackbar(title: "Ooops", message: 'Can\'t send message'));
  }

  setLastSentMessage({to, message}) async {
    DateTime currTime = DateTime.now();
    await lastmsgSetObject
        .update({
          'last_msg': {
            to: [
              {to: to, 'msg': message}
